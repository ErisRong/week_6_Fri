...
```{r}
new_stroke = stroke_mortality_raw %>% 
  rename("HB"="HBR")
new_stroke
joined_data = hb %>% 
  full_join(new_stroke, by ="HB") 
#OR
#joined = full_join(new_stroke, hb, by="HB")
...
first_table= joined_data %>% 
  filter(Year==2018,
         ...
         Sex != "All") %>% 
  group_by(HBName) %>% 
  select(Diagnosis, Sex, HBName, NumberOfDeaths, CrudeRate, EASR) %>% 
  gt()
first_table
# gt table cannot be group_by, only dataframe can be
```
...
#fmt_percent(columns = c(*, *),decimals=0, scale_value = FALSE)
